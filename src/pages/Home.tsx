...
import { ChevronLeft, ChevronRight, MapPin, Thermometer, Droplets, Wind, Cloud, Sun, CloudRain } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Card, CardContent } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { useToast } from "@/hooks/use-toast";
import { WeatherService, MarketDataService, getUserLocation, WeatherData, MarketData } from "@/lib/api-services";
  ...
  const requestLocation = async () => {
    try {
      ...
      const [currentWeather, forecast] = await Promise.all([
        WeatherService.getCurrentWeather(position.lat, position.lon),
        WeatherService.getWeatherForecast(position.lat, position.lon)
      ]);
      
      // Get location name
      const locationName = await WeatherService.getLocationFromCoords(position.lat, position.lon);
      ...
      toast({
        ...
        description: "Real-time weather forecast updated for your location",
      });
    } catch (error) {
      ...
      toast({
        ...
        description: "Unable to fetch weather data. Please check your internet connection.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
        ...
        {locationPermission === 'default' && (
          <Card className="p-6 text-center">
            ...
            <p className="text-muted-foreground mb-4">
              Get real-time weather forecasts for your farm location
            </p>
            <Button onClick={requestLocation} className="hover-glow" disabled={loading}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Card className="p-4 glass">
              <div className="text-center">
                ...
                <div className="text-lg font-semibold">Vijayawada</div>
              ...
              <Card key={item.crop} className="hover-lift">
                <CardContent className="p-4">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <Badge 
                      className={item.trend === "up" ? "bg-green-100 text-green-700" : item.trend === "down" ? "bg-red-100 text-red-700" : "bg-gray-100 text-gray-700"}
                    >
